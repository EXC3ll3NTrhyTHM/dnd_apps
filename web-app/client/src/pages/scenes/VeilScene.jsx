/**
 * VeilScene - The Veil spy network scene
 *
 * Custom scene for the_veil location with shadow/mystical theming
 */
...
export default function VeilScene({
  location,
  isAdmin,
  onNpcClick,
  onGatheringClick,
  ...
  return (
    <div
      className={`location-scene veil-scene ${editor.editMode ? 'edit-active' : ''}`}
      ref={editor.sceneRef}
    >
      {/* Background */}
      <div
      ...
      {/* Shadow wisps */}
      <div className="scene-wisps">
        {Array.from({ length: 12 }, (_, i) => (
          <div key={i} className={`wisp w${i + 1}`} />
        ))}
      </div>
